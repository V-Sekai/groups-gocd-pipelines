        ...
        jobs: [
          {
            ...
            tasks: [
                ...
                } else null,
              {
                ...
                arguments: [
                  '-c',
                  'rm -rf ' + export_info.export_directory + ' && mkdir ' + export_info.export_directory + ' && curl -L -O https://github.com/qarmin/gtk_library_store/releases/download/3.24.0/swiftshader.zip && unzip swiftshader.zip && rm swiftshader.zip && curr="$(pwd)/libvk_swiftshader.so" sed -i "s|PATH_TO_CHANGE|$curr|" vk_swiftshader_icd.json && chmod +x ' + 'godot.linuxbsd.opt.tools.64.llvm' + ' && HOME="`pwd`" VK_ICD_FILENAMES=vk_swiftshader_icd.json DRI_PRIME=0 --auto-servernum ./' + 'godot.linuxbsd.opt.tools.64.llvm' + ' --export "' + export_info.export_configuration + '" "`pwd`"/' + export_info.export_directory + '/' + export_info.export_executable + ' --path g -v',
                ],
                command: '/bin/bash',
                working_directory: '',
              },
