	...
	jobs: [
		{
			...
			artifacts: [{
				destination: ""
				source:      "g/bin/godot.macos.editor.double.x86_64"
				type:        "build"
			},
				{
					destination: ""
			...
			tasks: [
				...
				},
				{
					...
					arguments: ["-c", "mkdir -p ../.cicd_cache && OSXCROSS_ROOT=\".\" SCONS_CACHE=../.cicd_cache scons module_mvsqlite_enabled=off vulkan=no metal=yes osxcross_sdk=darwin23.3 werror=no platform=macos target=editor use_lto=no precision=double use_static_cpp=yes builtin_freetype=yes"]
					command:           "/bin/bash"
					type:              "exec"
					working_directory: "g"
				}, {
	...
	jobs: [{
	...
	},
		{
			...
			tasks: [{
				...
				job:              "linux_job"
				pipeline:         "godot-groups"
				source:           "godot.macos.editor.double.x86_64"
				stage:            "defaultStage"
				type:             "fetch"
			},
				{
...

				{
					...
					arguments: ["-c", "cp bin/godot.macos.editor.double.x86_64arm64 bin/macos_debug.x86_64 && cp bin/godot.macos.editor.double.x86_64 bin/macos_release.x86_64 && strip --strip-debug bin/macos_release.x86_64"]
					command:           "/bin/bash"
					type:              "exec"
					working_directory: "g"
				}, {
