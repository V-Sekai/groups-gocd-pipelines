...
local platform_info_dict = {
  windows: {
    ...
    godot_scons_arguments: "use_mingw=yes use_llvm=yes use_lld=yes use_thinlto=yes LINKFLAGS='-Wl,--enable-auto-import -Wl,--exclude-all-symbols -Wl,-pdb= CCFLAGS=\"-g -gcodeview\"' debug_symbols=no",
    extra_commands: [],
    ...
    platforms: {
      windows: {
        ...
        extra_commands: [
          ...
          'cd demo/addons/godot-openvr/bin/win64 && mv libgodot_openvr.dll libgodot_openvr.dbg.dll && mingw-strip --strip-debug -o libgodot_openvr.dll libgodot_openvr.dbg.dll && llvm-strip --wildcard --strip-symbol "*" --strip-symbol "!openvr_library_init" --strip-symbol "!deinitialize_level" --strip-symbol "!initialize_level" libgodot_openvr.dll libgodot_openvr.dbg.dll',
        ],
...
local itch_fire_template = [godot_template_groups_editor, godot_cpp_pipeline] + godot_gdextension_pipelines + [godot_template_groups_export] + [godot_template_groups] + [godot_template_hop_dance_export];
